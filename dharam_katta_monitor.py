...
def evaluate_frame(platform_polygon, vehicles, foreign, margin_px=10):
    ...
    # Check foreign objects on platform (persons, furniture, etc.)
    for obj in foreign:
        ...
        if inside_points:
            alarm = True
            reasons.append(
                f"ALARM: {obj['label']} (conf {obj['confidence']:.2f}) detected ON platform"
            )

    # Check vehicle boundary
    if not vehicles:
        if not alarm:
            reasons.append("OK: no vehicle detected on platform")
    else:
        for v in vehicles:
            ...
            if outside:
                alarm = True
                reasons.append(
                    f"ALARM: {v['label']} (conf {v['confidence']:.2f}) crosses platform boundary"
                )
            else:
                reasons.append(f"OK: {v['label']} (conf {v['confidence']:.2f}) within boundary")
...
def annotate(frame, platform_polygon, vehicles, foreign, alarm):
    ...
    for v in vehicles:
        ...
        cv2.putText(out, f"{v['label']} {v['confidence']:.2f}",
                    (x1, max(0,y1-8)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255,128,0), 2)
        for p in v["contact_points"]:
            cv2.circle(out, p, 6, (0,255,255), -1)
    ...
    for obj in foreign:
        ...
        cv2.putText(out, f"{obj['label']} {obj['confidence']:.2f}",
                    (x1, max(0,y1-8)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0,100,255), 2)
        for p in obj["contact_points"]:
            cv2.circle(out, p, 6, (0,50,255), -1)

    # Big status label
    label = "ALARM" if alarm else "OK"
    color = (0,0,255) if alarm else (0,200,0)
    cv2.rectangle(out, (10,10), (220,65), (0,0,0), -1)
    cv2.putText(out, label, (20,52), cv2.FONT_HERSHEY_SIMPLEX, 1.5, color, 3)
    return out
